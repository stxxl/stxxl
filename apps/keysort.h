#ifndef KEYSORT_HEADER
#define KEYSORT_HEADER

/***************************************************************************
 *            keysort.h
 *
 *  Fri Oct  4 20:43:57 2002
...
static void
count (type_key * a, type_key * aEnd, int *bucket, int K, unsigned offset,
       unsigned shift)
{
	// reset buckets
	for (int i = 0; i < K; i++)
...
static void
classify (type_key * a,
	  type_key * aEnd, type_key * b, int *bucket, unsigned offset, unsigned shift)
{
	for (type_key * p = a; p < aEnd; p++)
	{
		int i = (p->key - offset) >> shift;
...
l1sort (type_key * a,
	type_key * aEnd,
	type_key * b, int *bucket, int K, int offset, int shift)
{
