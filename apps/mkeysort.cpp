 ...
 *            mkeysort.cpp
 *
 *  Wed Sep 10 14:19:00 2002
 *  Copyright  2002  dementiev
...
gen_input (std::list < BID < B > >&input, int ndisks)
{
	...
	for (unsigned i = 0; i < nruns; i++)
	{
		...
		for (; j < _m; j++)
		{
			...
			{
				Blocks[j][k]._key = rand0K(1<<BITS);
				if(Blocks[j][k]._key >= (1<<BITS))
				{
						STXXL_ERRMSG("11 Key "<<Blocks[j][k]._key<<" is greater or equal than "<<(1<<BITS))
						abort();
				}
			}
	...
	if (_n % _m)
	{
		...
		for (; j < rest; j++)
		{
			for (int k = 0; k < Block::size; k++)
			{
				...
				if(Blocks[j][k]._key >= (1<<BITS))
				{
						STXXL_ERRMSG("12 Key "<<Blocks[j][k]._key<<" is greater or equal than "<<(1<<BITS))
						abort();
				}
			}
...
*/
     struct type_key
     {
	     Type::key_type key;
	     Type *ptr;
	     type_key (Type::key_type k, Type * p):key (k), ptr (p)
	     {
	     };
			 type_key() {};
     };
		 
		 inline bool operator < (const type_key & a, const type_key & b)
		 {
				 return a.key < b.key ;
		 };
...
//typedef std::pair< BID<B>,request *> next_run_read_entry;

struct next_run_read_entry
{
	Block  * block;
	BID<B> bid;
	request ** req;
...
void classify_block(Type * begin,Type * end,type_key * & out,int * bucket,unsigned offset, unsigned shift)
{
	for (Type * p = begin;p<end; p++,out++)	// count & create references
	{
		out->ptr = p;
		Type::key_type key = p->key();
...
typedef std::list < BID < B > >::iterator input_block_iterator;


void write_complete_handler(request * req, void * param)
{
	next_run_read_entry * next_read = (next_run_read_entry *)param;
	next_read->block->read(next_read->bid,*(next_read->req));
};


void put_to_pos(Type * pos, type_key * what)
{
	*pos = *(what->ptr);
};

void write_out(type_key *begin,type_key * end,Block *& cur_blk,const Block * end_blk,
					int & out_block,int & out_pos,Run & run,next_run_read_entry *& next_read,
					request ** write_reqs, request ** read_reqs,input_block_iterator & it)
{			
	block_manager *bm = block_manager::get_instance ();
	...
	{
		//elem[out_pos++] = *(p->ptr);
		put_to_pos(elem + out_pos++,p);
		
		if (out_pos >= Block::size)
		{
			run[out_block].key = cur_blk->elem->key();
			...
			if (cur_blk < end_blk)
			{
					...
					cur_blk->write (	run[out_block].bid, 
														write_reqs[out_block],
														write_complete_handler,								// queue read of block from next run
														(void*)(next_read++)); // after write of block from this run
...
void
create_runs (std::list < BID < B > >&input, Run ** runs, int nruns)
{
	const int m2 = _m / 2;
	block_manager *bm = block_manager::get_instance ();
	Block *Blocks1 = new Block[m2];
	Block *Blocks2 = new Block[m2];
	type_key *refs1 = new type_key[m2 * Block::size];
	type_key *refs2 = new type_key[m2 * Block::size];
	request **read_reqs = new request *[m2];
	request **write_reqs = new request *[m2];
	next_run_read_entry * next_run_reads = new next_run_read_entry[m2];
	const int offset = 0;
	const int log_k1 = 10;
	const int log_k2 = int(log2 (m2 * Block::size)) - log_k1 - 1;
	...
	input_block_iterator it = input.begin ();
	...
	/*	if(! read_reqs[i])
		...
		classify (refs1, refs1 + run_size * Block::size, refs2, bucket1,
			  offset, shift1);
		...
		type_key *c = refs2;
		type_key *d = refs1;
		Block *cur_blk = Blocks2;
		Block *end_blk = Blocks2 + next_run_size;
		next_run_read_entry * next_read = next_run_reads;
		...
		for (i = 0; i < k1; i++)
		{
			...
			write_out(d,dEnd,cur_blk,end_blk,
							out_block,out_pos,*run,next_read,
							write_reqs,read_reqs,it);
...
#define merge_runs merge_runs_lt
	...
	STXXL_MSG ("Elapsed time:" << end -
		   begin << " sec. Run creation time: " << after_runs_creation
		   - begin)
#ifdef STXXL_IO_STATS
		STXXL_MSG ("reads              : " << iostats->
			   get_reads ())STXXL_MSG ("writes             : " <<
						   iostats->
						   get_writes ())STXXL_MSG
		("read time          : " << iostats->
		 get_read_time () << " s") STXXL_MSG ("write time         : "
						      << iostats->
						      get_write_time () <<
						      " s")
		STXXL_MSG ("parallel read time : " << iostats->
			   get_pread_time () << " s")
		STXXL_MSG ("parallel write time: " << iostats->
			   get_pwrite_time () << " s")
		STXXL_MSG ("parallel io time   : " << iostats->
			   get_pio_time () << " s")
#endif
#ifdef COUNT_WAIT_TIME
		STXXL_MSG ("Time in I/O wait   : " << stxxl::
			   wait_time_counter << " s")
#endif
#define FINAL_CHECK
#ifdef FINAL_CHECK
		STXXL_MSG ("Checking order...") if (1)
	{
		...
		for (i = 0; i < _n; i++)
		{
			...
			{
				if (blk[0] < lastrecord)
					STXXL_MSG ("bug between blocks " << i
						   -
						   1 << " and " << i <<
						   ", keys " << lastrecord.
						   key () << ":" << blk[0].
