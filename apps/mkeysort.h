#ifndef MKEYSORT_HEADER
#define MKEYSORT_HEADER

/***************************************************************************
 *            mkeysort.h
 *
 *  Wed Sep 11 16:56:33 2002
...
struct Type
{
	...
	char filler[128 - sizeof (key_type)];
};
...
/*
class Run: public std::vector < TriggerEntry >
{
  Run() {};
public:
...
#define CHECK_RUN_BOUNDS(pos) \
         if(array[( pos )%dim_size]->size() <= (( pos ) / dim_size) ) \
         { \
	   STXXL_ERRMSG("Out of "<< (pos)%dim_size <<"th run bounds:" << pos/dim_size <<" >= " << array[(pos)%dim_size]->size()<<" pos:"<<pos ) \
           abort(); \
	 } 
*/
