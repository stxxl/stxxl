...
namespace stream
{
  ...
  public:
    ...
    runs_creator(Input_ & i,Cmp_ c,unsigned memory_to_use):
      input(i),cmp(c),m_(memory_to_use/(BlockSize_*STXXL_SORT_MEMORY_FACTOR)),result_computed(false)
    {
	assert (2*BlockSize_*STXXL_SORT_MEMORY_FACTOR <= memory_to_use);
    }
  ...
  public:
    ...
    runs_creator(Cmp_ c,unsigned memory_to_use):
      	cmp(c),
      	m_(memory_to_use/(BlockSize_*STXXL_SORT_MEMORY_FACTOR)),
