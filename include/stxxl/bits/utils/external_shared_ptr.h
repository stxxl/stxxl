...
/*
 * This class takes a shared pointer, increments its reference count and
...
private:
    /*
     * We store the pointer like this so that the refcount does not
     * get incremented when the wrapper is copy-constructed,
     * or decremented when the wrapper is destroyed.
     *
     * The whole external_shared_ptr object will be aligned by the compiler
     * to a multiple of its size. The size of the object is sizeof(P) as the
     * buffer is its only member. The buffer is placed in the class at offset
...
public:
    external_shared_ptr()
    {
         ...
         * for an object constructed this way the behavior is undefined.
         */
    }
