    ...
    Comparator comp)
{
    ...
    STXXL_EXPENSIVE_ASSERT(target_end == target + length - overhang);
    STXXL_EXPENSIVE_ASSERT(stxxl::is_sorted(target, target_end, comp));
    ...
    STXXL_EXPENSIVE_ASSERT(target_end == target + length);
    STXXL_EXPENSIVE_ASSERT(stxxl::is_sorted(target, target_end, comp));
    ...
    Comparator comp)
{
    ...
    const source_type k = static_cast<source_type>(seqs_end - seqs_begin);
    ...
    Comparator comp)
{
    ...
    for (RandomAccessIteratorIterator s = seqs_begin; s != seqs_end; ++s)
        STXXL_EXPENSIVE_ASSERT(stxxl::is_sorted((*s).first, (*s).second, comp));
    ...
    STXXL_EXPENSIVE_ASSERT(stxxl::is_sorted(target, target + length, comp));
...
template <
    ...
    typename RandomAccessIteratorIterator,
    typename Comparator>
void parallel_multiway_merge_sampling_splitting(
...
template <
    ...
    typename RandomAccessIteratorIterator,
    typename Comparator>
void parallel_multiway_merge_exact_splitting(
    const RandomAccessIteratorIterator& seqs_begin,
    const RandomAccessIteratorIterator& seqs_end,
    typename std::iterator_traits<typename std::iterator_traits<
                                      RandomAccessIteratorIterator>::value_type::first_type>::
    ...
    Comparator comp)
{
    ...
    for (RandomAccessIteratorIterator rii = seqs_begin; rii != seqs_end; ++rii)
        STXXL_EXPENSIVE_ASSERT(stxxl::is_sorted((*rii).first, (*rii).second, comp));
...
#pragma omp single
        ...
        sequential_multiway_merge<Stable, false>(
            ...
            std::min(local_length, static_cast<DiffType>(length) - target_position),
            comp);
    ...
    STXXL_EXPENSIVE_ASSERT(count_seqs == num_seqs);
