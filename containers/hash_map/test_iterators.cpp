...
//			assert((*cit).second == (*cit).first+1);
	...
	for (unsigned i = 0; i < n_tests; i++) {
		...
		const_iterator cit1 = cmap.find(key1); assert(cit1 != cmap.end());
		const_iterator cit2 = cmap.find(key2); assert(cit2 != cmap.end());
	...
	std::random_shuffle(values2.begin(), values2.end());
	for (unsigned i = 0; i < n_tests; i++) {
		const_iterator cit1 = cmap.find(values1[i].first); assert(cit1 != cmap.end());
		const_iterator cit2 = cmap.find(values2[i].first); assert(cit2 != cmap.end());
		iterator it1 = map.find(values1[i].first); assert(it1 != map.end());
		iterator it2 = map.find(values2[i].first); assert(it2 != map.end());
	...
	for (unsigned i = 0; i < n_tests; i++) {
		const_iterator cit1 = cmap.find(values1[i+n_tests].first); assert(cit1 != cmap.end());
		const_iterator cit2 = cmap.find(values2[i+n_tests].first); assert(cit2 != cmap.end());
